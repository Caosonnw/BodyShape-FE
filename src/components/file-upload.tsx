...
import { UploadCloud, File as FileIcon, Trash2, Loader, CheckCircle } from 'lucide-react'
import { useUploadVideoExerciseMutation } from '@/queries/useMedia'
...
export default function FileUpload({ value, onChange }: FileUploadProps) {
  const uploadMediaMutation = useUploadVideoExerciseMutation()
  ...
  const handleFiles = async (fileList: FileList) => {
    ...
    for (const f of newFiles) {
      ...
      try {
        ...
        formData.append('video', f.file as File)
        ...
        if (response?.payload?.data) {
          ...
          setFiles((prev) => prev.map((file) => (file.id === f.id ? { ...file, progress: 100 } : file)))
          onChange(response.payload.data)
        }
      } catch (error) {
  ...
  // Giới hạn tối đa 95% để chờ upload thật hoàn tất.
  const simulateUpload = (id: string) => {
    let progress = 0
    uploadIntervals.current[id] = setInterval(() => {
      progress += Math.random() * 10 + 5 // tăng 5-15% mỗi lần
  ...
  return (
    <div className='w-full max-w-3xl mx-auto p-4 md:p-6'>
      ...
      <motion.div
        ...
        className={clsx(
          'relative rounded-2xl p-8 md:p-12 text-center cursor-pointer bg-gray-100 border border-primary/10 shadow-sm hover:shadow-md backdrop-blur group',
        ...
        <div className='flex flex-col items-center gap-5'>
          ...
          <div className='space-y-2'>
            <h3 className='text-xl md:text-2xl font-semibold text-zinc-800 dark:text-zinc-100'>
              {isDragging ? 'Drop files here' : files.length ? 'Add more files' : 'Upload your files'}
            </h3>
            ...
            <p className='text-sm !text-zinc-500 dark:text-zinc-400'>Supports images, documents, videos, and more</p>
          </div>
      ...
      <div className='mt-8'>
        <AnimatePresence>
          {files.length > 0 && (
            ...
            >
              <h3 className='font-semibold text-xs md:text-xl !text-zinc-800 dark:text-zinc-200'>
          ...
          <AnimatePresence>
            {files.map((file) => (
              <motion.div
                ...
                className='px-4 py-4 flex items-start gap-4 rounded-xl !bg-gray-200 dark:bg-zinc-800/80 shadow hover:shadow-md transition-all duration-200'
              >
                {/* Thumbnail */}
